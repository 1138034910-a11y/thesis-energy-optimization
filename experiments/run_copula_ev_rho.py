"""Deterministic EV sweep for one rho value across 200/400/1000 t H2."""
import os
import sys
import time
import argparse
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/tables/copula_sensitivity_ev.csv")
    args = parser.parse_args()
    rho = args.rho

    print(f"=== Copula EV sweep: rho={rho} ===")
    t0_total = time.time()
    ...
    print(f"  scenarios generated in {time.time()-t0:.1f}s, weights={weights_sc}")
    ...
    for cap_h2 in H2_CAPACITIES:
        ...
        print(f"  H2={cap_t}t: Obj={ev_res['objval']:.2f}, Gap={ev_res['mipgap']:.4f}%, Time={ev_time:.1f}s, BESS_P={ev_res['capacity']['BESS_P_MW']:.0f}")
        results.append({
            "rho": rho,
            "H2_Tank_t": cap_t,
            "H2_Tank_kg": cap_h2,
