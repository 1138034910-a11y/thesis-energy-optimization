...
"""
================================================================================
SSE Sensitivity to Electrolyzer Cost Parameters (Robustness Check)
================================================================================
Purpose: Verify that the SSE three-stage sign-pattern (near-neutral →
         substitution → strong substitution) survives ±20% electrolyzer cost
...
Design:
  ...
  - All parameters identical to main experiment:
      ...
      * BESS cost at baseline
      * EV warm start (MIPStart) for every TSSP run
  - Only electrolyzer CAPEX (C_inv_ELC) is perturbed.
...
# Main-text baseline BESS power values for protocol-consistency check.
MAIN_TEXT_BESS_P = {
    200: 5518.92,
    400: 5758.33,
    600: 4061.03,
...
COST_SCENARIOS = [
    ("baseline",  1.00),   # matches main-text cost assumptions
    ("elc_low",   0.80),   # electrolyzer 20% cheaper
    ("elc_high",  1.20),   # electrolyzer 20% more expensive
]

ELC_COST_KEY = "C_inv_elc"  # must match config.py EXACTLY
...
def prepare_shared_data():
    ...
    print("=" * 70)
    print("SSE Sensitivity to Electrolyzer Cost (±20%) — Protocol-Consistent Version")
    print("=" * 70)
    ...
    if ELC_COST_KEY not in EconParams:
        raise KeyError(
            f"'{ELC_COST_KEY}' not found in EconParams. "
            f"Available keys: {list(EconParams.keys())[:10]}... "
            f"Please adjust ELC_COST_KEY."
        )
...
def run_single(job_spec, shared_data):
    ...
    baseline_elc_cost = EconParams[ELC_COST_KEY]
    econ_mod[ELC_COST_KEY] = baseline_elc_cost * elc_mult
    ...
    print(f"      ELC cost: {econ_base[ELC_COST_KEY]:.0f} "
          f"(×{elc_mult:.2f}, baseline={baseline_elc_cost:.0f})")
...
def main():
    ...
    for scenario_label, elc_mult in COST_SCENARIOS:
    ...
    for job in jobs:
        ...
        df_partial.to_csv(
            os.path.join(OUTPUT_DIR,
                         f"sse_elc_cost_sensitivity_partial_{ts}.csv"),
            index=False, encoding="utf-8-sig"
        )

    # --- Compute and compare SSE ---
    print("\n" + "=" * 70)
    print("SSE COMPARISON ACROSS ELECTROLYZER COST SCENARIOS")
    print("=" * 70)
    ...
    final_path = os.path.join(OUTPUT_DIR,
                               f"sse_elc_cost_sensitivity_{ts}.csv")
